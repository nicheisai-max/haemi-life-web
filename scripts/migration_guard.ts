...
/**
 * ENTERPRISE MIGRATION SAFETY GUARD (STAGED FILES ONLY — TS)
 * 
...
function runMigrationGuard() {
    console.log('🛡️  Running Migration Safety Guard (Staged Files Only)...');

    let stagedSqlFiles: string[] = [];
    try {
        // Get list of staged files that are SQL files
        const output = run_safe_command('git diff --cached --name-only');
