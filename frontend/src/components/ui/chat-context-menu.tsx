...
export const ChatContextMenu: React.FC<ChatContextMenuProps> = ({
    ...
    // 🩺 CRITICAL FIX: useClickOutside returns the ref that MUST be attached to the menu div.
    // Using a separate menuRef breaks the hook — it can never determine if clicks are inside.
    ...
    // Note: useClickOutside is now wired via the returned menuRef (see above).
    // No separate call needed — the ref is the bridge.
    ...
    const handleAction = (action: () => void) => {
        action();
        // Phase 17: Institutional Passive-Timing Guard
        // Standard: Google/Meta Grade (Zero-Race Architecture)
        // Goal: Ensure parent state registers mutation before unmount.
    ...
    return (
        <AnimatePresence>
            ...
            <motion.div
                ...
                className={`${isMobile ? 'fixed' : 'absolute'} z-[2000] haemi-ignore-click-outside ${isMobile
            ...
            >
                ...
                <div className={`flex items-center justify-between ${isMobile ? 'mb-8 px-1' : 'mb-2 p-1.5 bg-slate-50/60 dark:bg-white/5 rounded-xl'}`}>
                    {reactions.map((reaction) => (
                        <button
                            key={reaction.id}
                            onClick={() => handleAction(() => onReact(reaction.id))}
