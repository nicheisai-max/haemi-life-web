...
const BACKUP_DIR = path.join(__dirname, '..', '..', 'institutional_archives');
...
const notifyArchive = path.join(BACKUP_DIR, `notifications_archive_${timestamp}.sql`);
...
const commands = [
    ...
    {
        name: 'Notifications',
        cmd: `SET PGPASSWORD=Deepti@8143&& ${pgDumpPath} ${dbUrl} -t notifications > "${notifyArchive}"`
    }
];
